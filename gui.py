...
def _find_image():
    """Search common locations for the shop banner image."""
    search_dirs = [
        os.path.dirname(os.path.abspath(__file__)),   # same folder as script
        os.path.expanduser("~"),                        # user home
        r"C:\Codes\Projects\Ub",                        # original hardcoded path
        r"C:\ub",                                       # output folder
    ]
    names = ["img.jpg", "img.png", "img.jpeg", "banner.jpg", "banner.png"]
...
class Win1(tk.Tk):
    def __init__(self, logic):
        ...
        if img_path:
            try:
                ...
                tk.Label(self, text="Expense Tracker", bg="skyblue",
                         font=("Arial Black", 24)).pack(pady=10)
        else:
            tk.Label(self, text="Expense Tracker", bg="skyblue",
                     font=("Arial Black", 24)).pack(pady=10)

        self.entry = tk.Entry(self, textvariable=self.date_var,
                              font=("Arial", 12), state="readonly", width=14)
        self.submit = tk.Button(
            self, text="Select Date", width=10, height=1,
            background="blue", foreground="yellow", activebackground="lightblue",
            font=("Arial Black", 12), command=self.open_calendar,
        )
    ...
    def open_calendar(self):
        ...
        tk.Button(
            self.top, text="Submit", width=10, height=1,
            background="blue", foreground="yellow", activebackground="lightblue",
            font=("Arial Black", 12), command=self.interface,
        ).pack(pady=5)
    ...
    def interface(self):
        ...
        donebtn = tk.Button(self.frame, text="Store Data", width=13, height=1,
                            background="blue", foreground="yellow", activebackground="lightblue",
                            font=("Arial Black", 12), command=self.store)
        contbtn = tk.Button(self.frame, text="Add", width=5, height=1,
                            background="blue", foreground="yellow", activebackground="lightblue",
                            font=("Arial Black", 12), command=self.cont)
        killbtn = tk.Button(self.frame, text="Kill Entry", width=13, height=1,
                            background="blue", foreground="yellow", activebackground="lightblue",
                            font=("Arial Black", 12), command=self.kill)
        switchbtn = tk.Button(self.frame, text="Switch", width=10, height=1,
                              background="blue", foreground="yellow", activebackground="lightblue",
                              font=("Arial Black", 12), command=self.switch)
        rembtn = tk.Button(self.frame, text="Remove", width=7, height=1,
                           background="blue", foreground="yellow", activebackground="lightblue",
                           font=("Arial Black", 12), command=self.rem)
        calbtn = tk.Button(self.frame, text="Calculator", width=13, height=1,
                           background="blue", foreground="yellow", activebackground="lightblue",
                           font=("Arial Black", 12), command=self.calc)
        ...
        # Tab order: entry1 → entry2 → entry6 → entry3 → entry4 → entry5 → back to entry1
        # Enter on each field moves focus to the next field; does NOT trigger any button
        tab_order = [self.entry1, self.entry2, self.entry6, self.entry3, self.entry4, self.entry5]
        for i, field in enumerate(tab_order):
            next_field = tab_order[(i + 1) % len(tab_order)]
            field.bind("<Return>", lambda e, nf=next_field: nf.focus_set())
            field.bind("<KP_Enter>", lambda e, nf=next_field: nf.focus_set())
    ...
    def view(self):
        ...
        label = tk.Label(self.vframe, textvariable=contents,
                         background="skyblue", font=("Courier", 12))
    ...
    def store(self):
        state = self.logic.store(
            self.date_var.get(), self.entry1.get(), self.entry2.get(),
            self.entry3.get(), self.entry4.get(), self.entry5.get(),
            self.entry6.get(), self.purchase.get(),
        )
        if state == 1:
            msg.showwarning("Invalid Entry", "Please check your entries")
            return
        if state == 2:
            ch = msg.askyesno("File exists", f"Do you want to overwrite data on {self.date_var.get()}?")
            if not ch:
                return

        state = self.logic.store2(self.date_var.get())
        if state == 3:
            ch = msg.askyesno("Error", "Previous day file not found\nContinue with today as first?")
            if not ch:
                self.logic.kill()
                return

        state = self.logic.store3(
            self.entry1.get(), self.entry2.get(),
            self.entry6.get(), self.entry5.get(),
        )
        if state == 1:
            ...
            msg.showwarning(
                "File In Use",
                "Could not save — a Word document in C:\\ub is currently open.\n\n"
                "Please close all Word windows and click 'Store Data' again."
            )
        elif state == 0:
    ...
    def rem(self):
        ...
        if state == 1:
            msg.showwarning("Invalid operation", "No more data to be removed")
            return
        self.view()
        self.s_no.set(self.s_no.get() - 1)
        self.sourceCount.set(f"Source {self.s_no.get()}")
    ...
    def kill(self):
        ans = msg.askyesno("Warning", "Do you want to terminate entries?")
        if ans:
            self._reset_ui()
            self.logic.kill()
...
if __name__ == "__main__":
    class Test:
        def store(self, *a): print("store logic")
        def store2(self, *a): print("store2 executed")
        def cont(self, *a): print("continue logic")
        def kill(self): print("kill logic")
        def switch(self): print("switch logic")
        def rem(self): print("remove logic")
        def view(self):
            w1, w2, w3 = "Elephant", "Ant", "Dinosaur"
            return f"{w1:<32}{w2:<32}{w3:<32}"
